...
function resolvePeriodValue<T>(
  ...
  if (typeof value === "object" && value !== null && billingPeriod in value) {
    return (
      (value as Partial<Record<PricingBillingPeriod, T>>)[billingPeriod] ??
      fallback
    );
  }
    ...
    <section className="py-8 sm:py-16 lg:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-24 text-center">
          ...
          <div className="relative flex items-center justify-center gap-2">
            ...
            <div className="absolute top-10 left-1/2 flex translate-x-[50%] items-center gap-2">
              ...
              <Badge className="border-sky-600 text-sky-600" variant="outline">
                {yearlyDiscountLabel}
              </Badge>
            </div>
          </div>
        ...
        <div className="relative grid items-end gap-6 lg:grid-cols-3">
          {plans.map((plan) => {
            const ctaDisabled = resolvePeriodValue<boolean>(
              plan.ctaDisabled,
              billingPeriod,
              false
            );
            const ctaLabel = resolvePeriodValue<string>(
              plan.ctaLabel,
              billingPeriod,
              "Purchase Now"
            );
            ...
            return (
              <Card
                className={cn(
                  "relative w-full overflow-hidden sm:max-lg:mx-auto sm:max-lg:w-lg",
                  {
